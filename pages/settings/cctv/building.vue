<template>
    <SettingsCCTV type="building" @add="modalAdd = true"  v-model="search">
    ...
    <UModal v-model="modalAdd"  :ui="{ width: 'sm:max-w-7xl', height: 'min-h-7xl'}" @close="closeModal">
        <UForm :state="form" :schema="schema" @submit="submit">
            <UCard :ui="{ base: 'px-8', ring: '', divide: 'divide-y divide-black dark:divide-black' }">
                ...
                <UFormGroup label="อาคาร" name="description1" size="xl">
...
<script setup>
    ...
    const { data: lists, pending, refresh } = await useAsyncData(
        'lists',
        async () => {
            const data = await getMasterType(`BUILDING`, search.value)
    ...
    const form = ref({
        MasterTypeID: "BUILDING",
        ValueTXT: `BUILDING_${Math.random().toString(16).slice(2)}`,//value
    ...
    const schema = object({
        description1: string().required('กรอกอาคาร'),
    })
    ...
    const deleteItem = async () => {
        const res = await deleteMasterType({
            MasterTypeID:"BUILDING",//BUILDING , HD_CCTV_PURPOSE ,  HD_ITEMCATE , HD_ITEMTYPE ,HD_REPAIR_PURPOSE
