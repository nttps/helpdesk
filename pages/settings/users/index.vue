...
<script setup>
    ...
    const fetchGroups = async () => {
        const data = await getApi('/api/Person/ListUserGroup?appid=HELP DESK')
    ...
    const { data: lists, pending, refresh } = await useAsyncData(
        'lists',
        async () => {
            const data = await postApi('/api/Person/ListUserInfoWithPage', {
    ...
    const groupSubmit = async() => {
        if(!stateGroup.value) {
            const data = await deleteApi('/api/Person/DeleteUserFromGroup', {
                "Username": userGroup.value,
                "GroupID": selectGroup.value,
                "AppID":"HELP DESK"
            })
        }else {
            const data = await postApi('/api/Person/AddUserToGroup', {
                "Username":  userGroup.value,
                "GroupID":selectGroup.value,
                "AppID":"HELP DESK"
            })
    ...
    const fetchEditData = async (value) => {
        const data = await getApi(`/api/Person/ListUserInGroup?username=${value.username}&appid=HELP DESK`)
