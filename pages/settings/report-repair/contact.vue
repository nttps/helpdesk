    ...
    <UModal v-model="modalAdd"  :ui="{ width: 'sm:max-w-7xl', height: 'min-h-7xl'}" @close="closeModal">
        <UForm :state="form" :schema="schema" @submit="submit">
            <UCard :ui="{ base: 'px-8', ring: '', divide: 'divide-y divide-black dark:divide-black' }">
                ...
                <UFormGroup label="ช่องทางการติดต่อ" name="description1" size="xl">
...
<script setup>
    ...
    const { data: lists, pending, refresh } = await useAsyncData(
        'lists',
        async () => {
            const data = await getMasterType(`HD_CONTACT`, search.value)
    ...
    const form = ref({
        MasterTypeID: "HD_CONTACT",
        ValueTXT: `HD_CONTACT_TYPE${Math.random().toString(16).slice(2)}`,
        description1: "", 
        description2: ""
    })

    const schema = object({
        description1: string().required('กรอกรายละเอียดนายหน้า'),
    })
    ...
    const closeModal = () => {
        form.value = {
            MasterTypeID: "HD_CONTACT",
            ValueTXT: `HD_CONTACT_TYPE${Math.random().toString(16).slice(2)}`,//value
